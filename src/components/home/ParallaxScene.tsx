...
// ── Sticky card shell ─────────────────────────────────────────────────────────
// Each section sits inside a 100vh scroll anchor.
// The inner card is position:sticky so it pins below the navbar while the
// next card slides up from below — creating the "deck of cards" overlap.
function StackCard({
  children,
  index,
}: {
  ...
  return (
    // scroll anchor — gives this card 100vh of scroll distance
    <div style={{ height: "100vh" }}>
      <div
        style={{
          position: "sticky",
          top: 64,                              // pins flush under the navbar
          height: "calc(100vh - 64px)",
          zIndex: 20 + index,                   // each card covers the previous
        }}
      >
        {/* Opaque card — covers the card below it as it slides up */}
        <div
          className="w-full h-full bg-hacker-bg border-t border-neon-green/10 overflow-y-auto"
          style={{
        ...
        >
          ...
          <div className="flex items-center justify-center min-h-full px-4 sm:px-6 lg:px-8 py-10">
            <motion.div
...
// Reusable terminal prompt bar inside a card
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-5">
          {[
            ...
            <div key={label}>
              <p className="text-[10px] text-neon-green/40 tracking-widest mb-2 font-mono">
                {label}
              </p>
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] px-3 py-1 rounded border border-neon-green/15 text-neon-green/60 font-mono hover:border-neon-green/50 hover:text-neon-green hover:bg-neon-green/5 transition-all duration-200 cursor-default"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1.5">
                {entry.tags?.map((tag) => (
                  <span
                    key={tag}
                    className="text-[10px] px-2 py-0.5 rounded border border-neon-green/10 text-neon-green/50 font-mono group-hover:border-neon-green/25 transition-colors duration-200"
                  >
                    {tag}
                  </span>
                ))}
...
const sections = [IntroBlurb, AboutBrief, ExperienceBrief, ProjectsPreview, CTASection];

export default function ParallaxScene() {
  return (
    <div className="w-full">

      {/* ── Hero — full viewport, background effects, unchanged ── */}
      <div
      ...
      {/* ── Stacked cards — each slides over the previous as you scroll ── */}
